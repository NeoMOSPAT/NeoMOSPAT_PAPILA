...
def grids_within_radius(mini, mesh, radius, method_distance="cos"):
    ######################################################
...
#(obs.apply(lambda x: np.nansum(mini["W"].values*(x.values-modelstation.loc[x.name].values)), axis=1).to_frame(name=var)/obs.apply(lambda x: np.nansum(mini["W"].values*(x.values/x.values)), axis=1).to_frame(name=var) ).head() 
#(obs.apply(lambda x: np.nansum(mini["distance_factor"].values*mini["W"].values*(x.values-modelstation.loc[x.name].values)), axis=1).to_frame(name=var)/obs.apply(lambda x: np.nansum(mini["W"].values*(x.values/x.values)), axis=1).to_frame(name=var) ).head()
######################################################
...
def SCM_estimate(mini, model, obs, modelstation, epsilon=1):
    ...
    sum_up=obs.apply(lambda x: np.nansum(mini["distance_factor"].values*mini["W"].values*(x.values-modelstation.loc[x.name].values)), axis=1).to_frame(name=var) 
...
def main(t_ObsStationData, t_ModelStationData, t_ModelData, t_Stations_Info, t_ModelStation_Info, t_Models_Info, window=10, radius=10):
    ...
    new_models={}
    new_info={}
    c_BC_NewNames=[c+"_SCM_DIST" for c in IncF.c_BC_NewNames]
