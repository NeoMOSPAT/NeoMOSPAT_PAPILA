...
def main(t_ObsStationData, t_Stations_Info, t_ModelStationData,  t_ModelStation_Info,  t_ModelData, t_Model_Filters, t_Models_Info):
    ...
    for count, plot_freq in enumerate(IncF.c_Map_freq):
        ...
        for vars_plot in IncF.c_Map_mosaic:
            ...
            for model in list(vars_to_plot["M"])+["STD", "MAD"]:
                ## Vars that need to be done 
                if model in ["STD", "MAD"]:
                    vars_model=vars_to_plot["M"]["CAMS"]
                    model_info=t_Models_Info["CAMS"]
                    units=t_ModelData[freq]["CAMS"].meta.units
                    filters=t_Model_Filters["CAMS"]
                else:
                ...
                data_resampled=t_ModelData[freq][model].reset_index(level=[0,1]).sort_values(by=["i_lat", "i_lon"]) 
                ...
                if type_col=="time":
                    ...
                    for start, end in zip(IncF.c_Start_Date, IncF.c_Last_Date):
                        ...
                        df_period = data_resampled.loc[d_start.strftime("%Y-%m-%d"):d_end.strftime("%Y-%m-%d")]
                        data[f"{d_start.strftime('%Y-%m-%d')}-{d_end.strftime('%Y-%m-%d')}"] = df_period.groupby(["i_lat", "i_lon"]).mean().reset_index()
                        cols.append(f"{d_start.strftime('%Y-%m-%d')}-{d_end.strftime('%Y-%m-%d')}")
                ...
                if type_row=="Vars":
                    nrow=len(vars_model)
                    rows=list(vars_model)
                for type_plot in ["Square", "CTRF"]:
                    for scale in ["Normal", "Log"]:
                        fig, axes = plt.subplots(nrows=nrow, ncols=ncol, sharex=True, sharey='row', figsize=(14/2*nrow+1,9*ncol))
                        if nrow==1:
                            axes=np.array([axes])
                        for i in range(nrow):
                            row = rows[i]
                            for j in range(ncol):
                                col = cols[j]
                                data = df.loc[col]
                                data = data.merge(model_info, on=["i_lat", "i_lon"], how="outer").sort_values(by=["i_lon", "i_lat"])
                                z = data[row].values.reshape((len_x, len_y)) 
                                unit=units[row]
                                if type_plot=="Square":
                                    if scale=="Log":
                                        vmax=np.nanpercentile(df[row], 99)
                                        vmin = np.nanmin(df[row].values[np.nonzero(df[row].values)])
                                        norm=LogNorm(vmin=vmin, vmax=vmax)
                                    else:
                                        vmax=np.nanpercentile(df[row], 97)
                                        vmin=0
                                ...
                                if type_plot=="CTRF":
                                    ...
                                    if scale=="Log":
                                        ...
                                        levels[0] = 0
                                        levels = list(levels[np.isfinite(levels)])
                                        nlevels=len(levels)
                                    ...
                                    #norm = BoundaryNorm(np.linspace(vmin, vmax, 13), ncolors=256)
                                ...
                                if col=="2015-01-01-2015-01-31" or col=="2015-01-01-2015-02-01":
                                    col="Jan"
                                if col=="2015-07-01-2015-07-31" or col=="2015-07-01-2015-08-01":
                                    col="Jul"
                                axes[i,j].set_title("%s %s  %s [%s]"%(model, col, row, aux_plots.get_pretty_units(unit)), pad=10)
                            ...
                            if scale=="Log" and type_plot=="Square":
                                ...
                                formatter=LogFormatterExponent(base=2)
                                fig.colorbar(cs, cax=cax, extend="max", spacing='uniform', ticks=locator, format=formatter)
                            else:
                                fig.colorbar(cs, cax=cax, extend="max", spacing='uniform')

                        fig.subplots_adjust(wspace=0, hspace=0)
                        aux_plots.savefigure(fig, path_freq, f"{model}_{''.join(rows)}_{type_plot}_{scale}.png")
