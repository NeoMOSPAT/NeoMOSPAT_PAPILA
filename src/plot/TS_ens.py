...
def plot_basic(name, data_or, color_dict, timeframe, var1, var2, units, direc="", legend_title="", highlight=""):
    ...
    if not data_2.empty:
        data_2[[data_2.columns[0]]].plot(color=color2_0, ax=ax2, style=ls, ms=ms)

    df_min=data_2.min(axis=1)
    df_max=data_2.max(axis=1)
    ax2.plot(data_2.index, df_max.values, color=color2_0, alpha=0.1, label='_nolegend_', ls="-")
    ax2.plot(data_2.index, df_min.values, color=color2_0, alpha=0.1, label='_nolegend_', ls="-")
    ax2.fill_between(data_2.index, df_max, df_min, color=color2_0, alpha=0.3)
